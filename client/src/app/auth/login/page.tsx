...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await axios.post('http://localhost:7000/auth/login', {
        identifier,
        password,
      });
  ...
  return (
    ...
    <div className="w-full flex items-center justify-center min-h-screen bg-gray-900 font-[Inter]">
      <div
        className={`bg-gray-800 text-white p-8 rounded-xl shadow-2xl max-w-md w-full ${
          shake ? 'shake' : ''
        }`}
      >
        {/* Login form title. */}
